...
async def submit_slip(request: Request, slip: UploadFile = File(...)):
    ...
    caption = f"🔔 **ငွေလွှဲပြေစာ အသစ် ရောက်ရှိပါသည်!**\n\n👤 User: {user_email}\n📦 Package: Premium Package (ကျပ် ၅,၀၀၀)\n📅 နေ့စွဲ: {date.today()}"

    telegram_url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto"
    async with httpx.AsyncClient() as client:
        files = {"photo": (slip.filename, slip_bytes, slip.content_type)}
        data = {
            "chat_id": TELEGRAM_CHAT_ID,
            "caption": caption,
            "reply_markup": '{"inline_keyboard": [[{"text": "✅ Approve (+10 Credits)", "callback_data": "approve_' + user_email + '"}, {"text": "❌ Reject", "callback_data": "reject"}]]}'
        }
        await client.post(telegram_url, data=data, files=files)

    return {"status": "success", "message": "ပြေစာ ပေးပို့ပြီးပါပြီ။ Admin အတည်ပြုပြီးပါက Credits တိုးပေးပါမည်။"}
    ...
    try:
        ...
        if "callback_query" in update:
            ...
            if data.startswith("approve_"):
                target_email = data.replace("approve_", "")
                db = SessionLocal()
                target_user = db.query(User).filter(User.email == target_email).first()
                if target_user:
                    target_user.package_credits += 10
                    db.commit()
                db.close()
                ...
                async with httpx.AsyncClient() as client:
                    await client.post(
                        ...
                        json={"callback_query_id": cb["id"], "text": f"{target_email} အား 10 Credits ပေးပြီးပါပြီ!"}
                    )
    except Exception:
        pass
    return {"ok": True}
...
async def batch_generate_tts(request: Request, payload: BatchTTSRequest):
    user_email = request.cookies.get("user_email")
    if not user_email:
        return JSONResponse(status_code=401, content={"error": "Export ပြုလုပ်ရန် Login ဝင်ပေးပါခင်ဗျာ။"})
    ...
    if user.daily_credits_left > 0:
        ...
        db.close()
        return JSONResponse(status_code=403, content={"error": "ယနေ့အတွက် အခမဲ့ ၂ ပုဒ် ကုန်သွားပါပြီ။ Premium Package ဝယ်ယူပေးပါခင်ဗျာ။"})
